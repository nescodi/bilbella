...
- **Animations**: plain CSS transitions/keyframes + a small `IntersectionObserver`-driven `FadeIn` component (`components/FadeIn`) — no animation library. This is deliberate: on `output: "export"`, a JS-dependent library whose `initial` state is invisible (e.g. `opacity: 0`) can leave content permanently blank if hydration is ever delayed or fails on some static host. `FadeIn` is built the opposite way — content is visible by default in the raw HTML, and JS only hides-then-reveals it via direct DOM `classList` toggling (never React state) once it's confirmed running, so a JS failure degrades to "visible, unanimated" rather than "invisible forever." Keep motion subtle and purposeful (fades, slides, gentle scale) — nothing bouncy or cartoonish — and always pair a CSS animation/transition with a `prefers-reduced-motion: reduce` override.
